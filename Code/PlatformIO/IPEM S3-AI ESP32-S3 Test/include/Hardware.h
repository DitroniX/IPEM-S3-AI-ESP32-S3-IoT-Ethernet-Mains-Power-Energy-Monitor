...
// Application
String AppVersion = "260817"; // Internal Firmware Date Code
...
// Constants USER
const int LoopDelay = 5;                         // Loop Delay in Seconds.  Default 1.
...
#define RXLP 18
#define TXLP 17
#define ENLP 45 // RS485 UART Enable

// **************** ESP32-S3 OUTPUTS ****************
#define ATM_RST 16 // ATM Reset

#define OPTO_OUT 46 // OPTO_OUT
#define LED_Red 2   // Red LED
#define LED_RGB 48  // RGB LED

// **************** ESP32-S3 INPUTS ****************
#define User_Button 0

// #define MISO 5	// SPI MISO
// #define MOSI 6	// SPI MOSI
// #define SCLK 7 // SPI SCLK

// #define CS_ATM 4 // SPI CS
...
#define LED_GPIO LED_RGB   // change this number to be the GPIO pin connected to the LED
#define LED_BRIGHT 15      // sets how bright the LED is. O is off; 255 is burn your eyeballs out (not recommended)
#define LED_COUNT 2        // set the number of LEDs.
static const crgb_t L_RED = 0xff0000;
static const crgb_t L_GREEN = 0x00ff00;
static const crgb_t L_BLUE = 0x0000ff;
static const crgb_t L_WHITE = 0xe0e0e0;
LiteLED RGB(LED_TYPE, LED_TYPE_IS_RGBW);
...
// Current.  Remember to set the Jumpers on EACH channel to CT Clamp for the below examples.
// This is calculated based on the common CT Clamp YDHC 100A/50mA  Need to allow for +/- ~ 1% Tolerance.  Trim values are fine!
// Calculations: Base value for CT100/50 Mid Range 33500.  To increase/decrease change by Approx. ~500 per 0.01A RMS
// Maximum gain is 65535 = 0xFFFF
// See Examples below on CT Clamp values.  A base difference will be needed for 90E32
...
// Voltage and Current - Overall Gain
// PMPGA 0x17              DDV3V2V1I4I3I2I1  // D = DPGA       4/3/2/1 = V1-4 = I1-4
unsigned short PGAGain = 0b0101010101010101; // PMPGA 0x17  | DPGA Gain = 2 and PGA Gain = 1 | 0x5555
...
#if ATM_SINGLE_PHASE == true
unsigned short VoltageGain1 = 20200;        // uGain = UgainA | 20200 Default Starting Value | 0x4EEB
unsigned short VoltageGain2 = VoltageGain1; // Duplicate V1 Values to V2 and V3.
unsigned short VoltageGain3 = VoltageGain1; // Duplicate V1 Values to V2 and V3.
#else
unsigned short VoltageGain1 = 20200; // uGain = UgainA | 20200 Default Starting Value | 0x4EEB
unsigned short VoltageGain2 = 20200; // uGain = UgainB | 20200 Default Starting Value | 0x4EEB
unsigned short VoltageGain3 = 20200; // uGain = UgainC | 20200 Default Starting Value | 0x4EEB
#endif
...
// Example Calibrated Values, using SCT013-000-100A-50mA as a benchmark.
// 33500  0x82DC  SCT013-000-100A-50mA (YDHC) (Jack)
// 33000  0x80E8  DL-CT03CL2.0 24A-2.5mA
// 59000  0xE678  OPCT10AL 60A-1A
// 16500  0x4074  PZCT-02
// 16500  0x4074  DL-CT08CL6
// 16500  0x4074  OPCT10AL
// 57350  0x4074  OPCT10AL (Jack)
// 41500  0xA21C  OPCT16AL
// 16500  0x4074  GTA20L1.0
// 33100  0x4074  DL-CT08CL5
// 16500  0x4074  GTA3UL-30
// 16500  0x4074  DL-CT08CL2
// 33000  0x80E8  CT08-CL10

unsigned short CurrentGainCT1 = 33500; // IgainA 0x62 > 0x82DC = 33500
unsigned short CurrentGainCT2 = 33500; // IgainB 0x65 > 0x82DC = 33500
unsigned short CurrentGainCT3 = 33500; // IgainC 0x6A > 0x82DC = 33500
unsigned short CurrentGainCTN = 33500; // IgainD 0x6E > 0x82DC = 33500
#endif
...
#if ATM90DEVICE == ATM90E32_DEVICE
    OLEDPrint("ATM90E32AS", 2, 2);
#endif
#if ATM90DEVICE == ATM90E36_DEVICE
    OLEDPrint("ATM90E36A", 2, 2);
#endif
...
{
  // Cycle the LEDs
  RGB.brightness(LED_BRIGHT, LED_COUNT); // Turn the LED On

  RGB.setPixel(0, L_RED, LED_COUNT);
  delay(400);

  RGB.setPixel(0, L_GREEN, LED_COUNT);
  delay(400);

  RGB.setPixel(0, L_BLUE, LED_COUNT);
  delay(400);

  RGB.setPixel(0, L_WHITE, LED_COUNT);
  delay(400);

  RGB.brightness(0, LED_COUNT); // Turn the LED Off
} // RGB
...
void DisplayHeader()
{
  ...
  Serial.printf("ESP32-S3 Serial ID: %04X", (uint16_t)(chipid >> 32));
  Serial.printf("%08X", (uint32_t)chipid);
  Serial.println("");
  Serial.print(AppAcronym + " Firmware Version: ");
...
void ConfigureBoard()
{
  ...
  // Initialise ATM_RST on GP16
  pinMode(ATM_RST, OUTPUT);
  digitalWrite(ATM_RST, HIGH);
  ...
  Wire.setClock(100000);              // try 100 kHz first if 400 kHz fails

  // Initialize WS2812B
  RGB.begin(LED_GPIO, LED_COUNT);
  RGB.brightness(LED_BRIGHT, LED_COUNT); // turn the LED on
...
// Display Hardware Configuration for ATM90E32 ATM90E36
...
#else
#if ATM_SPLIT_PHASE == false // World Single, Three Phase or 3 x Single Phases
  ...
  if (sys0 == 65535 || sys0 == 0)
  {
    ...
    Serial.println(" Energy Meter - Check Device or SPI Connections/Configuration*");
    Serial.println("*********************************************************************");
    Serial.println("*********************************************************************");
  }
...
void DisplayCurrentCalibrationRegisters()
{
  ...
  PrintSeparator("Voltage 1: " + String(LineVoltage1) + " V\n");
  ...
  Serial.println("Current 1: " + String(LineCurrentCT1) + " A");

  Serial.println("Current 2: " + String(LineCurrentCT2) + " A");

  Serial.println("Current 3: " + String(LineCurrentCT3) + " A");
