...
// Calculate Voltage
  ...
  Serial.println("* SOFTWARE CONFIGURATION ERROR *\n* You cannot have ATM_SINGLE_PHASE and ATM_SPLIT_PHASE, both Enabled.\n");
#else
...
#if ATM_SINGLE_PHASE == false

    Serial.println("Voltage 1: " + String(LineVoltage1) + " V");

#if ATM_SPLIT_PHASE == false
  ...
  if (DisplayFull == true) // Display Expanded Information
  {
    ...
    Serial.println("\nCurrent CTN: " + String(LineCurrentCTN) + " A");
#endif
...
#if ATM_SPLIT_PHASE == true // USA 120+120
...
void setup()
{
  ...
  Serial.println("IPEM S3-AI - Integrated Firmware for DitroniX Power Energy Monitor SDK Boards\n");
...
#if ATM90DEVICE == ATM90E32_DEVICE && ATM_SPLIT_PHASE == true // Not Fully Tested.
// the 2nd (B) current channel is not used with the split phase meter
  ...
  Serial.println("ATM_SINGLE_PHASE\n");
#else
  eic.begin(LineFreq, PGAGain, VoltageGain1, VoltageGain2, VoltageGain3, CurrentGainCT1, 0, CurrentGainCT2);
  Serial.println("ATM_SPLIT_PHASE\n");
#endif
#endif

#if ATM90DEVICE == ATM90E32_DEVICE && ATM_SPLIT_PHASE == false
#if ATM_SINGLE_PHASE == true
  ...
  // Display Optional ATM Specific Registers

  // Specific Register Information
  // eic.GetRegisters();

  // Useful Information when calibrating Voltage and Current
  eic.GetCalibrationRegisters();
...
void loop() // This loop tests the various GPIO ports and also outputs read data.
{
  ...
  Serial1.write("Hello IPEM World - Test\n");
